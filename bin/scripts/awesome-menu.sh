...
function music {
	launcher='rofi -dmenu -width 330 -theme slate'
...
function wallpaper {
	...
	unsplash () {
		...
		chosen=$(rofi -i -dmenu -l 0 -width 330 -theme slate -p 'Enter search term: ')
...
function updates {
	...
	show_updates () {
		if [[ -z $(checkupdates) ]]; then
			...
			list=$(echo "$(checkupdates)" | $rofi -l $HEIGHT -i -p "$(echo "$NUM") updates " )
		fi
...
function menu {
	...
	options="$power\n$screenshot\n$music\n$wallpapers\n$todo\n$video\n$updates\n$other"
	opt="$(echo -e "$options" | $ROFICOMMAND -width 550 -dmenu -selected-row 0)"
	case $opt in
		$power)
		...
		$wallpapers)
			...
			;;
		$todo)
			$HOME/projects/rofi-menus/todo.sh
			;;
		$video)
