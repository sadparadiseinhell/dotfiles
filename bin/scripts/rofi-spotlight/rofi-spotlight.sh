...
then
	case "${1}" in
		...
		"Open file location in ${TERM_EMU}" )
			...
			coproc ( ${TERM_EMU} -e bash -c "cd "${file_path%/*}" ; ${SHELL}" & > /dev/null 2>&1 )
			kill -9 $(pgrep rofi)
			;;
		"Open file location in ${FILE_MANAGER}" )
			file_path="$(cat "${CURRENT_FILE}")"
...
function context_menu_icons() {
	...
	then
		echo "\0icon\x1fsystem-file-manager\n"
...
function context_menu() {
	...
	elif [[ "${type}" == "image/jpg" ]] || [[ "${type}" == "image/jpeg" ]] || [[ "${type}" == "image/png" ]]
	then
